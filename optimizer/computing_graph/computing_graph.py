...
def get_computation_graph(model: tf.keras.Model, optimizer=keras.optimizers.Adam(3e-4),
    ...
    if isinstance(model, Sequential):
        ...
        input_ids_constraint = tf.TensorSpec(shape=[batch_size, 128], dtype=tf.int32, name="input_ids")
        attention_mask_constraint = tf.TensorSpec(shape=[batch_size, 128], dtype=tf.int32, name="attention_mask")
        labels_constraint = tf.TensorSpec(shape=[batch_size, 128], dtype=tf.int32, name="labels")
        concrete_function = training_step.get_concrete_function(input_ids_constraint, labels_constraint,
                                                                attention_mask_constraint)
    else:
