...
def near_optimal_scheduling(model: Model, start, finish, comm_start, comm_end, comp_graph: CompGraph,
    ...
    for device, subgraph in device_subgraph_mapping.items():
        ...
        for high_cost_node in high_cost_nodes:
            non_connected_chain_list = get_non_connected_list_by_operator(comp_graph, high_cost_node)
            for op_a, op_b in itertools.combinations(non_connected_chain_list, 2):
