...
def split_four_stage_subgraph(subgraph: CompGraph, operator_device_mapping, edge_cut_list) -> tuple[
    ...
    independent_non_exporting_nodes = set(node for node in non_exporting_node if len(get_dependent_node_set(node)) == 0)
    dependent_non_exporting_nodes = non_exporting_node - independent_non_exporting_nodes
    ...
    # stage two is what we need to optimize
    stage_two = subgraph.subgraph(dependent_relied)
    stage_three = subgraph.subgraph(independent_non_exporting_nodes)
    stage_four = subgraph.subgraph(dependent_non_exporting_nodes)

    assert len(subgraph.nodes) == len(stage_one.nodes) + len(stage_two.nodes) + len(stage_three.nodes) + len(stage_four.nodes)
